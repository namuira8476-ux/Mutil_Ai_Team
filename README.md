...
**여러 AI를 하나의 팀으로.** Codex·Claude·Gemini(agy)를 연필 그림 캐릭터로 보여주는 로컬 AI 작업실입니다.
...
현재 **0.1.16 개발 미리보기**, Windows x64를 지원합니다. macOS·Linux·ARM 설치파일은 제공하지 않습니다. 설치파일은 코드서명되지 않았습니다. 내장 브라우저 직접 제어는 현재 Codex만 지원합니다. 토큰·한도는 CLI가 제공하는 범위에서만 표시합니다.
...
릴리스 담당자는 package.json 버전과 일치하는 `v0.1.16` 형태의 태그를 푸시합니다. GitHub Actions가 Windows에서 설치·검사·빌드 후 설치파일과 체크섬을 Releases에 게시합니다. GitHub 자동 생성 소스 ZIP도 같은 릴리스에서 받을 수 있습니다.
